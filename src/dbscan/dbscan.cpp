...
Cluster::add_hit(Hit* h)
{
  ...
  if (h->connectedness == Connectedness::kCore && (!latest_core_point || h->time > latest_core_point->time)) {
    latest_core_point = h;
  }
}
...
IncrementalDBSCAN::add_hit(Hit* new_hit, std::vector<Cluster>* completed_clusters)
{
  ...
  if (clusters_neighbouring_hit.empty()) {
    ...
    if (new_hit->neighbours.size() + 1 >= m_minPts) {
      // std::cout << "New cluster starting at hit time " << new_hit->time << " with " << new_hit->neighbours.size() <<
      // " neighbours" << std::endl;
      ...
      // std::cout << "New hit time " << new_hit->time << " with " << new_hit->neighbours.size() << " neighbours is
      // noise" << std::endl;
    }
  } else {
    ...
    // std::cout << "Adding hit time " << new_hit->time << " with " << new_hit->neighbours.size() << " neighbours to
    // existing cluster" << std::endl;
  ...
  for (auto& neighbour : new_hit->neighbours) {
    if (neighbour->neighbours.size() + 1 >= m_minPts) {
      // std::cout << "new_hit's neighbour at " << neighbour->time << " has " << neighbour->neighbours.size() << "
      // neighbours, so is core" << std::endl;
      ...
      // std::cout << "new_hit's neighbour at " << neighbour->time << " has " << neighbour->neighbours.size() << "
      // neighbours, so is NOT core" << std::endl;
    }
  }
...
IncrementalDBSCAN::trim_hits()
{
  ...
  auto last_it = std::lower_bound(m_hits.begin(), m_hits.end(), earliest_time - 10 * m_eps, time_comp_lower);
