...
void
TriggerActivityMakerADCSimpleWindow::operator()(const TriggerPrimitive& input_tp,
...
TriggerActivityMakerADCSimpleWindow::configure(const nlohmann::json& config)
{
  ...
  TLOG_DEBUG(TRACE_NAME) << "If the total ADC of trigger primitives with times within a " << m_window_length
                         << " tick time window is above " << m_adc_threshold << " counts, a trigger will be issued.";
}
